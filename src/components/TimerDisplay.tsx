...
export function TimerDisplay({ lanes, laneStates, isRunning, trackLength = 5.5 }: TimerDisplayProps) {
  ...
  return (
    <div className="space-y-3">
      {enabledLanes.map((lane) => {
        ...
        const lastSpeed = lastLap ? calculateSpeed(lastLap, trackLength) : 0;
        const bestSpeed = state?.bestLap ? calculateSpeed(state.bestLap, trackLength) : 0;
